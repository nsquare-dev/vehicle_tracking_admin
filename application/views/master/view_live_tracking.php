...
<script async defer
    ...
    <script>
        ...
        function init2() {
            ...
            var myLatlng = new google.maps.LatLng(parseFloat(flightPlanCoordinates[flightPlanCoordinates.length-1].lat), parseFloat(flightPlanCoordinates[flightPlanCoordinates.length-1].lng));
            var mapOptions = {
                zoom: 16,
                center: myLatlng,
                zoomControl: true,
...
//                            polylineCoordinates.push(position);
                            ...
                            google.maps.event.addListener(marker, 'click', (function (marker, i) {
                                return function () {
                                    contentString = '<div>You are here</div>';
                                    infowindow.setContent(contentString);
                                    infowindow.open(map, marker);
                                }
